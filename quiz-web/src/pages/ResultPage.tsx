...
import { CheckCircleIcon, XCircleIcon, AwardIcon, BarChart2Icon, ArrowRightCircleIcon, RefreshCcwIcon } from 'lucide-react';
const ResultPage: React.FC = () => {
  ...
  // Check if state exists, if not redirect to quizzes
  if (!location.state) {
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div className={`px-6 py-8 text-center ${resultInfo.bgColor}`}>
            <div className="mx-auto w-16 h-16 rounded-full bg-white flex items-center justify-center mb-4">
              {isPassing ? <CheckCircleIcon className="h-10 w-10 text-green-500" /> : <XCircleIcon className="h-10 w-10 text-red-500" />}
            </div>
          ...
          <div className="p-6">
            ...
            <div className="mb-8">
              <div className="w-full bg-gray-200 rounded-full h-4">
                <div className={`${isPassing ? 'bg-green-600' : 'bg-red-600'} h-4 rounded-full`} style={{
                width: `${percentage}%`
              }}></div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4">
              <Link to={`/module/${moduleId}/quizzes`} className="inline-flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                <RefreshCcwIcon className="h-4 w-4 mr-2" />
                Try Another Quiz
              </Link>
              <Link to={`/module/${moduleId}/quiz/${quizId}`} className="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">
                <RefreshCcwIcon className="h-4 w-4 mr-2" />
                Retake This Quiz
              </Link>
            </div>
